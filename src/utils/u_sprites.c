...
int get_spr_pixel(char *spr_data, int tex_x, int tex_y, t_spr_system *sprs)
{
	...
	if (tex_x < 0 || tex_x >= sprs->grass_width || tex_y < 0 || tex_y >= sprs->grass_height)
		return 0xFF00FF;
...
void grass_loop_generating(t_game *game, t_minimap *minimap, t_spr_system *sprs, int i)
{
	if (get_map_value(&game->maps, minimap->map_x, minimap->map_y, game) == 0)
	{
		...
		while(i < sprs->grass_per_tile && game->sprs.spr_count < sprs->max_sprs)
		{
			...
			game->sprs.sprs[game->sprs.spr_count].x = minimap->rand_x;
			game->sprs.sprs[game->sprs.spr_count].y = minimap->rand_y;
...
void generate_grass_sprs(t_game *game, t_minimap *minimap)
{
	...
	while(minimap->map_y < game->maps.height)
	{
		...
		minimap->map_y++; 
	printf("Generated %d grass sprs\n", game->sprs.spr_count);
	}
}

int load_grass_texture(t_game *game)
{
	game->sprs.grass_img[0] = mlx_xpm_file_to_image(game->mlx.mlx, "textures/grass.xpm", 
	...
	game->sprs.grass_img[1] = mlx_xpm_file_to_image(game->mlx.mlx, "textures/grass2.xpm", 
													   &game->sprs.grass_width, &game->sprs.grass_height);
